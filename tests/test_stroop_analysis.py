...
def test_session_stats_all_timeouts(tmp_path: Path) -> None:
    ...
    df = load_session(csv_path)
    stats = session_stats(df)
    assert math.isnan(stats["avg_rt_congruent"])
    assert math.isnan(stats["avg_rt_neutral"])
    assert math.isnan(stats["avg_rt_incongruent"])
    assert math.isnan(stats["facilitation_ms"])
    assert math.isnan(stats["interference_ms"])
    assert math.isnan(stats["stroop_effect_ms"])
