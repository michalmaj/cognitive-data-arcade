...
def test_menu_l_key_toggles_language_and_persists(tmp_path: Path) -> None:
    ...
    scene.handle_event(pygame.event.Event(pygame.KEYDOWN, key=pygame.K_l, mod=0, unicode="l"))
...
def test_menu_z_key_launches_stroop_picker(tmp_path: Path) -> None:
    ...
    scene = _make_menu(tmp_path)
    scene.handle_event(pygame.event.Event(pygame.KEYDOWN, key=pygame.K_z, mod=0, unicode="z"))
    assert scene.is_done()
