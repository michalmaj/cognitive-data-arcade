...
class RecommendationBubbleScene(Scene):
    ...
    def _build_next_scene(self) -> Scene:
        ...
        score_curator = getattr(self._state, "score_curator", 0)
        ap = min(100, score_curator)
        stars = 3 if score_curator >= 65 else (2 if score_curator >= 40 else 1)
        session = SessionResult(
            task_name="recommendation_bubble",
            participant_id=self._pm.load().device_uuid,
            session_id="bubble_session",
            total_trials=3,
