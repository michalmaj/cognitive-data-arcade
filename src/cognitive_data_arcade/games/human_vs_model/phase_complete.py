...
from cognitive_data_arcade.games.human_vs_model.challenge_data import CompleteChallenge
...
class PhaseCompleteScene(Scene):
    def __init__(
        self,
        challenges: list[CompleteChallenge],
        round_idx: int,
        session_score: int,
        beat_ai_count: int,
    ) -> None:
    ...
    def _advance(self) -> None:
        ...
        if self._round_idx < len(self._challenges) - 1:
            ...
            self._next = PhaseResultScene(session_score=new_score, beat_ai_count=new_beat)
        self._done = True
    ...
    def _draw_top_bar(self, surface: pygame.Surface) -> None:
        ...
        phase_lbl = get_font(13).render("HARD (A)", True, _AMBER)
    ...
    def _draw_left_panel(self, surface: pygame.Surface) -> None:
        ...
        task_lbl = get_font(11).render("UZUPELNIJ ZDANIE", True, _DIM)
        surface.blit(task_lbl, (16, _TOP_H + 34))

        # Stem text (word-wrapped)
        y = _TOP_H + 60
        words = self._challenge.stem.split()
        line_buf = ""
        for word in words:
        ...
        if self._state == "reveal":
            if self._round_score > 0:
                ...
                surface.blit(delta, (16, _TOP_H + 430))
    ...
    def _draw_right_panel(self, surface: pygame.Surface) -> None:
        ...
        if self._state == "task":
            ...
            if self._beat_ai:
                beat = get_font(14).render("POBILES AI!  +15 bonus", True, _AMBER)
