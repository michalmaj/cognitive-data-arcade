...
class PhaseBScene(Scene):
    ...
    def _resimulate(self) -> None:
        self._student_result = simulate(
            self._tabs.dist_type, self._get_params(), rng_seed=self._seed
        )
        self._seed += 1
        self._student_surf = _render_chart(self._student_result, alpha=False)
        if self._target:
            score = match_score(
                self._tabs.dist_type,
                self._get_params(),
                self._target.dist_type,
                self._target.params,
            )
            self._success = score >= _SUCCESS_THRESHOLD

    def _button_rects(self) -> list[pygame.Rect]:
        return [pygame.Rect(_BX0 + i * (_BW + 8), _BY, _BW, _BH) for i in range(3)]
    ...
    def draw(self, surface: pygame.Surface, offset_y: int = 0) -> None:
        ...
        if self._target:
            ...
            area.blit(
                font.render(f"Dopasowanie: {score:.0f}%", True, score_col),
                (_LEFT_W + 8, _AREA_H - 90),
            )

        if self._success:
            area.blit(
                get_font(18).render("Świetnie! Dalej >>", True, _GREEN), (_LEFT_W + 8, _AREA_H - 60)
            )

        # Info block: left panel, below sliders (max slider y ~ 296)
        info_y = 310
        if self._hints_used >= 1 and self._target:
            area.blit(font.render(f"Typ: {self._target.dist_type}", True, _ORANGE), (16, info_y))
            info_y += 22
        ...
        area.blit(
            font.render(f"Wskazówki: {self._hints_used}/{_MAX_HINTS}", True, _DIM),
            (16, _AREA_H - 82),
        )
        ...
        for i, (lbl, enabled) in enumerate(
            ...
            tw, th = font.size(lbl)
            area.blit(
                font.render(lbl, True, _WHITE if enabled else _DIM),
