...
class PhaseResultScene(Scene):
    ...
    def _build_next_scene(self) -> "Scene":
        ...
        missions_done = len(self._round_results)
        ap = min(100, 50 + missions_done * 10)
        session = SessionResult(
            task_name="semantic_space_explorer",
            participant_id=self._pm.load().device_uuid,
            session_id="semantic_session",
            total_trials=missions_done,
            correct_trials=sum(1 for r in self._round_results if r.get("correct", False)),
