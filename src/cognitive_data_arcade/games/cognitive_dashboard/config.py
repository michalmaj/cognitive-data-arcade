...
def generate_synthetic(n_trials: int = MINI_TRIALS) -> DashboardSession:
    ...
    s_rts = _sample_rts(SYNTHETIC_PARAMS["stroop"]["congruent"], half, rng) + _sample_rts(
        SYNTHETIC_PARAMS["stroop"]["incongruent"], half, rng
    )
    ...
    f_rts = _sample_rts(SYNTHETIC_PARAMS["flanker"]["congruent"], half, rng) + _sample_rts(
        SYNTHETIC_PARAMS["flanker"]["incongruent"], half, rng
    )
    ...
    nogo_rts = [
        float(np.clip(rng.normal(250, 50), 100, 500)) if not c else -1.0 for c in nogo_correct
    ]
