...
CONTENT: dict[str, dict[str, list[str]]] = {
    "pl": {
        "theory": [
            ...
            "Istotność statystyczna != istotność praktyczna. Przy dużym N nawet trywialny efekt (Cohen's d = 0.05) daje p < 0.001. Zawsze pytaj: 'Jak duży jest ten efekt?' — nie tylko 'Czy jest istotny?'",
            "Cohen's d — miara wielkości efektu. Cohen's d = (M1 - M2) / s_pooled. Skala: pomijalny d < 0.10, mały 0.10-0.30, średni 0.30-0.50, duży 0.50-0.80, bardzo duży >= 0.80. Efekt d = 0.20 jest realny, ale w praktyce może być bez znaczenia.",
            "Moc testu i planowanie próby. Moc (1 - beta) to prawdopodobieństwo wykrycia efektu gdy on istnieje. Standardem jest moc 0.80. Zbyt małe N -> przeoczasz prawdziwe efekty (błąd II rodzaju). Zbyt duże N -> wykrywasz trywialne efekty i wyciągasz mylące wnioski.",
        ],
        "notes": [
    ...
    "en": {
        "theory": [
            "What does p-value actually mean? The p-value is the probability of observing data at least as extreme as yours, assuming H0 is true. p < 0.05 does not mean the effect is large or important — it only means the result would be unlikely if there were no effect.",
            "Statistical significance != practical significance. With large N, even a trivial effect (Cohen's d = 0.05) yields p < 0.001. Always ask: 'How large is this effect?' — not just 'Is it significant?'",
            ...
            "Statistical power and sample planning. Power (1 - beta) is the probability of detecting an effect when it truly exists. The standard is 80% power. Too small N -> you miss real effects (Type II error). Too large N -> you detect trivial effects and draw misleading conclusions.",
        ],
        "notes": [
