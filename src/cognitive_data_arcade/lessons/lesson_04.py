"""Lesson 04 - Data Quality Lab (Data Cleaning)."""
...
CONTENT: dict[str, dict[str, list[str]]] = {
    "pl": {
        "theory": [
            "Surowe dane to hipoteza o tym, co się wydarzyło. Są produktem sprzętu o skończonej precyzji, oprogramowania z opóźnieniami i uczestników, którzy mrugają i naciskają złe klawisze. Wyczyszczone dane to najlepsze dostępne przybliżenie rzeczywistości - konstruowane przez zastosowanie jawnych, udokumentowanych kryteriów.",
            "Zasada GIGO (Garbage In, Garbage Out) pochodzi z mainframe'ów IBM z lat 50. XX w. i przypisywana jest George'owi Fuechselowi. W kognitywistyce obowiązuje wprost: model statystyczny na brudnych danych daje błędne wnioski niezależnie od jego złożoności.",
            ...
            "Kryteria wykluczenia RT: odpowiedzi < 100 ms to antycypacje - mózg nie zdążył przetworzyć bodźca. Odpowiedzi > 2000-3000 ms to przerwy uwagi. Próby bez odpowiedzi (timeout) to trzecia kategoria - nie wolne odpowiedzi, ale nieobecne. Mieszanie tych trzech kategorii prowadzi do systematycznych błędów.",
            ...
            "Odtwarzalne czyszczenie oznacza skrypt, który można uruchomić ponownie z surowych danych i zawsze daje ten sam wynik. Ręczna edycja CSV w Excelu jest niewidoczna dla kontroli wersji i niemożliwa do odtworzenia. Skrypt Python z pandas tworzy ścieżkę audytu.",
            "Przypadek Stapela (2011) - holenderski psycholog Diederik Stapel sfałszował dane w ok. 55 pracach. Jednym z sygnałów ostrzegawczych była zbyt czysta statystyka: brak wartości odstających, nierealistycznie niskie odchylenia standardowe. Nadmiernie perfekcyjne dane są tak samo podejrzane jak dane ze zbyt dużą ilością błędów.",
        ...
        "notes": [
            "Błąd kontra wykluczona próba - nieprawidłowe odpowiedzi to prawidłowe dane dla analiz dokładności. Nie należy ich wykluczać z liczników dokładności. Błędne odpowiedzi można wykluczyć z analiz RT, jeśli analizie podlega wyłącznie szybkość odpowiedzi poprawnych - ale warunek ten musi być jawnie sformułowany.",
            "Agresywne czyszczenie to też problem - usuwanie zbyt wielu prób redukuje wariancję i może sztucznie zmienić rozkład RT. Czyszczenie motywowane zasadami biologicznymi (antycypacje, przerwy) różni sie od czyszczenia motywowanego chęcią uzyskania lepszych wyników.",
        ...
        "tasks": [
            ...
            "Wpisz w Pythonie: załaduj CSV przez pandas, zastosuj trzy maski (mask_anticipation, mask_lapse, mask_timeout) i wydrukuj: Razem / Wykluczone (%) / Wlaczone. Sprawdź, czy liczby zgadzają sie z ręcznym liczeniem.",
    ...
    "en": {
        "theory": [
            "Raw data is a hypothesis about what happened. It is the product of hardware with finite precision, software with scheduling latency, and participants who blink and press the wrong key. Cleaned data is the best available approximation of reality - constructed by applying explicit, documented criteria.",
            "The GIGO principle (Garbage In, Garbage Out) originates from IBM mainframe computing in the 1950s and is attributed to George Fuechsel. In cognitive science it applies directly: a statistical model on dirty data produces wrong conclusions regardless of its sophistication.",
            ...
            "RT exclusion criteria: responses < 100 ms are anticipations - the brain had not yet processed the stimulus. Responses > 2000-3000 ms are attentional lapses. Trials with no response (timeout) are a third category - not slow responses, but absent ones. Conflating these three categories introduces systematic error.",
            ...
            "Reproducible cleaning means a script that can be re-run from raw data and always produces the same output. Manual CSV editing in Excel is invisible to version control and cannot be reproduced. A Python pandas script creates the audit trail.",
            "The Stapel case (2011) - Dutch psychologist Diederik Stapel fabricated data in approximately 55 papers. One warning sign was overly clean statistics: no outliers, unrealistically low standard deviations. Suspiciously perfect data is as much a red flag as data with too many errors.",
        ...
        "notes": [
            "Error vs. excluded trial - incorrect responses are valid data for accuracy analyses. Do not exclude them from accuracy counts. Incorrect responses may be excluded from RT analyses when only correct-response speed is under analysis - but this must be stated explicitly.",
            "Aggressive cleaning is also a problem - removing too many trials reduces variance and can artificially shift the RT distribution. Cleaning motivated by biological principles (anticipations, lapses) differs from cleaning motivated by a desire for better-looking results.",
