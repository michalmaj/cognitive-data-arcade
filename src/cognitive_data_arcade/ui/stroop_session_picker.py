...
class _SessionEntry:
    ...
    date_str: str
    avg_rt: float
    stroop_effect: float  # incongruent_avg − congruent_avg (ms)
    accuracy: float
    n_trials: int
    n_correct: int
    bins: list[float]
...
class StroopSessionPickerScene(Scene):
    def __init__(
        self,
        sessions_dir: Path,
        strings: Strings,
    ...
    def _parse(self, path: Path) -> _SessionEntry:
        ...
        rts_by_cond: dict[str, list[float]] = {
            "congruent": [],
            "neutral": [],
            "incongruent": [],
        }
    ...
    def _open_analysis(self) -> None:
        ...
        chart = figure_to_surface(fig, (680, 520))
        self._next = StroopAnalysisScene(chart, stats, self._strings, back_scene=self)
        self._done = True
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        title = self._font_title.render(self._strings.stroop_picker_title, True, _WHITE)
    ...
    def _draw_rows(self, surface: pygame.Surface, w: int) -> None:
        ...
        for i, entry in enumerate(self._sessions):
            ...
            pygame.draw.rect(
                surface,
                _ROW_ACTIVE_BG if active else _ROW_BG,
                (0, y, w, _ROW_H - 1),
            )
            if active:
