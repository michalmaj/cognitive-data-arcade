...
def load_session(csv_path: Path) -> pd.DataFrame:
    """Load and normalise a Visual Search CSV."""
...
def session_stats(df: pd.DataFrame) -> dict[str, float]:
    ...
    Timeouts (rt_ms is NaN or <= 0) are excluded from RT means but counted
    ...
    def _mean_rt(cond: str) -> float:
        subset = df.loc[
            (df["condition"] == cond) & df["correct"] & (df["rt_ms"] > 0),
            "rt_ms",
        ]
    ...
    feat_rt = _mean_rt("feature")
    conj_rt = _mean_rt("conjunction")

    return {
        "feature_mean_rt": feat_rt,
        "conjunction_mean_rt": conj_rt,
        "search_cost_ms": conj_rt - feat_rt,
        "feature_accuracy": _accuracy("feature"),
        "conjunction_accuracy": _accuracy("conjunction"),
...
def build_comparison_chart(df: pd.DataFrame, figsize: tuple[float, float] = (7.0, 4.5)) -> Figure:
    """Two-bar chart: feature vs conjunction mean RT for correct trials."""
    stats = session_stats(df)
    labels = ["Feature", "Conjunction"]
    values = [stats["feature_mean_rt"], stats["conjunction_mean_rt"]]
    ...
    ax.set_title("Visual Search Effect", color="#f0f0f0")
